...
export const useProductsInfiniteQuery = (filters = {}, searchTerm = '') => {
  ...
  // Calculate filtering efficiency for debugging
  const filteringEfficiency = allProducts.length > 0 
    ? (filteredProducts.length / allProducts.length * 100).toFixed(1)
    : 100;

  console.log(`🔍 Filtering: ${allProducts.length} → ${filteredProducts.length} products (${filteringEfficiency}% match)`);

  return {
    // Data
    products: filteredProducts,
    allProducts, // For debugging
    ...
    filteringEfficiency: `${filteringEfficiency}%`,
    rawDataCount: allProducts.length,
    filteredDataCount: filteredProducts.length
  };
};
