...
interface UseProductsInfiniteQueryResult {
  products: Product[];
  isLoading: boolean;
  isFetching: boolean;
...
export const useProductsInfiniteQuery = (
  filters: Filters = {}, 
  searchTerm: string = ''
): UseProductsInfiniteQueryResult => {
  ...
  const infiniteQuery = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }: { pageParam?: number }): Promise<ProductsPage> => {
      let result;
  ...
  const allProducts = infiniteQuery.data?.pages?.flatMap((page: ProductsPage) => page.products) || [];
